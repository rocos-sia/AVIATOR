...
class ClearanceTrajectory {
  public:
    ...
    explicit ClearanceTrajectory(const fs::path &path, double d_safe, double q_margin, int trace_half,
        ...
        // PIN-IK builds its kinematic chain from the URDF directly (the dev branch
        // removed kdl_parser).  We reuse the MuJoCo-derived joint limits with the
        // J2 planning margin, exactly as the original TRAC-IK path did.
        std::ifstream urdf_in(resolve("urdf"));
        if (!urdf_in)
            throw std::runtime_error("Cannot read URDF");
        const std::string urdf_xml((std::istreambuf_iterator<char>(urdf_in)),
                                   std::istreambuf_iterator<char>());
        for (int s = 0; s < 2; s++) {
            auto side = std::string(s ? "right" : "left"),
                 prefix = std::string("AR5-5_07") + (s ? "R" : "L") + "-W4C4A2_";
            pinocchio::Model model;
            Eigen::VectorXd urdf_lower, urdf_upper;
            pinocchio::FrameIndex tip_frame;
            PIN_IK::loadURDFModel(urdf_xml, "aircraft", prefix + "flan_link", model,
                                  urdf_lower, urdf_upper, tip_frame);
            ...
            ik[s] = std::make_unique<PIN_IK::PIN_IK>(model, lower[s], upper[s], tip_frame, .005, 7e-7, PIN_IK::Speed);
