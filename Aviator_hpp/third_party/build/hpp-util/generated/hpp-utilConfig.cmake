...
if(NOT hpp-util_FOUND)
  set(skip_this_file FALSE)
endif()
if(skip_this_file)
  foreach(component ${hpp-util_FIND_COMPONENTS})
    if(NOT "hpp-util_${component}_FOUND")
      set(skip_this_file FALSE)
    endif()
  endforeach()
endif()
...
set("hpp-util_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("HPP_UTIL_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("hpp-util_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/hpp-util/doxygen-html")
set(
  "HPP_UTIL_DOXYGENDOCDIR"
  "${PACKAGE_PREFIX_DIR}/share/doc/hpp-util/doxygen-html"
)
set("hpp-util_DEPENDENCIES" "Boost;TinyXML2")
set("hpp-util_PKG_CONFIG_DEPENDENCIES" "")

set(
  CMAKE_MODULE_PATH
  ${CMAKE_MODULE_PATH}
  
  ${PACKAGE_PREFIX_DIR}/lib/cmake/hpp-util/find-external/TinyXML
)
...
set("_hpp-util_PKG_CONFIG_LIBDIR" "${pcfiledir}/../../lib")
set("_hpp-util_PKG_CONFIG_LIBS_LIST" " -lhpp-util")
if(_hpp-util_PKG_CONFIG_LIBS_LIST)
  string(FIND ${_hpp-util_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  while(_is_comma_space GREATER -1)
    string(
      REPLACE ", "
      ","
      _hpp-util_PKG_CONFIG_LIBS_LIST
      "${_hpp-util_PKG_CONFIG_LIBS_LIST}"
    )
    string(FIND ${_hpp-util_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  endwhile()
  ...
  set(LIBDIR_HINTS ${_hpp-util_PKG_CONFIG_LIBDIR})
  foreach(component ${_hpp-util_PKG_CONFIG_LIBS_LIST})
  ...
  endforeach()
endif(_hpp-util_PKG_CONFIG_LIBS_LIST)

set("hpp-util_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
set("HPP_UTIL_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
...
if(COMMAND ADD_REQUIRED_DEPENDENCY)
  foreach(pkgcfg_dep ${hpp-util_PKG_CONFIG_DEPENDENCIES})
...
include("${CMAKE_CURRENT_LIST_DIR}/hpp-utilTargets.cmake")
...
foreach(component ${hpp-util_FIND_COMPONENTS})
  ...
  if(EXISTS ${comp_file})
    ...
    set(hpp-util_${component}_FOUND FALSE)
  endif()
  if(hpp-util_${component}_FOUND)
    message(STATUS "hpp-util: ${component} found.")
  else()
    if(hpp-util_FIND_REQUIRED_${component})
      message(FATAL_ERROR "hpp-util: ${component} not found.")
    else()
      message(STATUS "hpp-util: ${component} not found.")
    endif()
  endif()
endforeach()
check_required_components("hpp-util")
