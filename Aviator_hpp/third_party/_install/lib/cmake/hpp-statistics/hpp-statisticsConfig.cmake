...
if(NOT hpp-statistics_FOUND)
  set(skip_this_file FALSE)
endif()
if(skip_this_file)
  foreach(component ${hpp-statistics_FIND_COMPONENTS})
    if(NOT "hpp-statistics_${component}_FOUND")
      set(skip_this_file FALSE)
    endif()
  endforeach()
endif()
...
set("hpp-statistics_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("HPP_STATISTICS_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("hpp-statistics_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/hpp-statistics/doxygen-html")
set(
  "HPP_STATISTICS_DOXYGENDOCDIR"
  "${PACKAGE_PREFIX_DIR}/share/doc/hpp-statistics/doxygen-html"
)
set("hpp-statistics_DEPENDENCIES" "hpp-util")
set("hpp-statistics_PKG_CONFIG_DEPENDENCIES" "")
...
set("_hpp-statistics_PKG_CONFIG_LIBDIR" "${pcfiledir}/../../lib")
set("_hpp-statistics_PKG_CONFIG_LIBS_LIST" " -lhpp-statistics")
if(_hpp-statistics_PKG_CONFIG_LIBS_LIST)
  string(FIND ${_hpp-statistics_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  while(_is_comma_space GREATER -1)
    string(
      REPLACE ", "
      ","
      _hpp-statistics_PKG_CONFIG_LIBS_LIST
      "${_hpp-statistics_PKG_CONFIG_LIBS_LIST}"
    )
    string(FIND ${_hpp-statistics_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  endwhile()
  ...
  set(LIBDIR_HINTS ${_hpp-statistics_PKG_CONFIG_LIBDIR})
  foreach(component ${_hpp-statistics_PKG_CONFIG_LIBS_LIST})
  ...
  endforeach()
endif(_hpp-statistics_PKG_CONFIG_LIBS_LIST)

set("hpp-statistics_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
set("HPP_STATISTICS_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
...
if(COMMAND ADD_REQUIRED_DEPENDENCY)
  foreach(pkgcfg_dep ${hpp-statistics_PKG_CONFIG_DEPENDENCIES})
...
include("${CMAKE_CURRENT_LIST_DIR}/hpp-statisticsTargets.cmake")
...
foreach(component ${hpp-statistics_FIND_COMPONENTS})
  ...
  if(EXISTS ${comp_file})
    ...
    set(hpp-statistics_${component}_FOUND FALSE)
  endif()
  if(hpp-statistics_${component}_FOUND)
    message(STATUS "hpp-statistics: ${component} found.")
  else()
    if(hpp-statistics_FIND_REQUIRED_${component})
      message(FATAL_ERROR "hpp-statistics: ${component} not found.")
    else()
      message(STATUS "hpp-statistics: ${component} not found.")
    endif()
  endif()
endforeach()
check_required_components("hpp-statistics")
