...
if(NOT hpp-pinocchio_FOUND)
  set(skip_this_file FALSE)
endif()
if(skip_this_file)
  foreach(component ${hpp-pinocchio_FIND_COMPONENTS})
    if(NOT "hpp-pinocchio_${component}_FOUND")
      set(skip_this_file FALSE)
    endif()
  endforeach()
endif()
...
set("hpp-pinocchio_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("HPP_PINOCCHIO_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("hpp-pinocchio_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/hpp-pinocchio/doxygen-html")
set(
  "HPP_PINOCCHIO_DOXYGENDOCDIR"
  "${PACKAGE_PREFIX_DIR}/share/doc/hpp-pinocchio/doxygen-html"
)
set("hpp-pinocchio_DEPENDENCIES" "Boost;hpp-util;pinocchio;Eigen3;example-robot-data")
set("hpp-pinocchio_PKG_CONFIG_DEPENDENCIES" "")
...
set("_hpp-pinocchio_PKG_CONFIG_LIBDIR" "${pcfiledir}/../../lib")
set("_hpp-pinocchio_PKG_CONFIG_LIBS_LIST" "")
if(_hpp-pinocchio_PKG_CONFIG_LIBS_LIST)
  string(FIND ${_hpp-pinocchio_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  while(_is_comma_space GREATER -1)
    string(
      REPLACE ", "
      ","
      _hpp-pinocchio_PKG_CONFIG_LIBS_LIST
      "${_hpp-pinocchio_PKG_CONFIG_LIBS_LIST}"
    )
    string(FIND ${_hpp-pinocchio_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  endwhile()
  ...
  set(LIBDIR_HINTS ${_hpp-pinocchio_PKG_CONFIG_LIBDIR})
  foreach(component ${_hpp-pinocchio_PKG_CONFIG_LIBS_LIST})
  ...
  endforeach()
endif(_hpp-pinocchio_PKG_CONFIG_LIBS_LIST)

set("hpp-pinocchio_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
set("HPP_PINOCCHIO_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
...
if(COMMAND ADD_REQUIRED_DEPENDENCY)
  foreach(pkgcfg_dep ${hpp-pinocchio_PKG_CONFIG_DEPENDENCIES})
...
include("${CMAKE_CURRENT_LIST_DIR}/hpp-pinocchioTargets.cmake")
...
foreach(component ${hpp-pinocchio_FIND_COMPONENTS})
  ...
  if(EXISTS ${comp_file})
    ...
    set(hpp-pinocchio_${component}_FOUND FALSE)
  endif()
  if(hpp-pinocchio_${component}_FOUND)
    message(STATUS "hpp-pinocchio: ${component} found.")
  else()
    if(hpp-pinocchio_FIND_REQUIRED_${component})
      message(FATAL_ERROR "hpp-pinocchio: ${component} not found.")
    else()
      message(STATUS "hpp-pinocchio: ${component} not found.")
    endif()
  endif()
endforeach()
check_required_components("hpp-pinocchio")
