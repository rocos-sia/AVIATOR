...
namespace joystick {
Device::Device(std::string path, bool probe_rumble)
    : path_(std::move(path)), probe_rumble_(probe_rumble) {}
...
const State& Device::poll() {
    if (fd_ < 0) {
        ...
        if (probe_rumble_) openRumble();
    }
    if (probe_rumble_ && rumble_fd_ < 0 &&
        std::chrono::steady_clock::now() >= retry_rumble_) openRumble();
    js_event event{};
    // 有界批量读取，避免持续输入独占调用线程。
