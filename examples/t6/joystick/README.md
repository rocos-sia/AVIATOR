...
`poll()` 以非阻塞方式读取排队事件并维护最新快照；返回引用在下次调用时更新。
默认还会探测力反馈设备，失败时每 2 秒重试，探测可能耗时数毫秒。
实时控制循环可用 `joystick::Device stick("/dev/input/js0", false)` 关闭力反馈探测；
`aviatorT6Joystick` 已使用此模式。
