...
安装入口为 [../scripts/install_dependencies.sh](../scripts/install_dependencies.sh)，支持 `--dry-run`。
...
| mujoco_deps/ccd、qhull | libccd-dev、libqhull-dev |
| mujoco_deps/tinyxml2、tinyobjloader | libtinyxml2-dev、libtinyobjloader-dev |
...
- `pinocchio/`（3.9.0）、`coal/`（3.0.4）、`pin_ik/`（2.2.0，包含本地适配）。
- `mujoco/`（3.4.0），以及 `mujoco_deps/` 中的 lodepng、marchingcubecpp、trianglemeshdistance；Jammy 官方仓库没有对应可替换开发包。
- `xcore/`：0.7.1 厂商头文件和 Linux x86_64 静态库。
