...
int main(int argc, char** argv) {
  try {
    ...
    for (const auto& axis : {std::string("1 0 0"), std::string("0 1 0"),
      ...
      PIN_IK_NLOPT::NLOPT_IK nlopt(cm, cl, cu, cf, 0.1, 1e-6);
    ...
    // Aviator regression: narrow J2-like limits and seeds displaced by whole turns.
    rotated_lo[0] = 85.5 * M_PI / 180.;
    rotated_hi[0] = 94.5 * M_PI / 180.;
    for (auto mode : {PIN_IK::Speed, PIN_IK::Distance}) {
      PIN_IK::PIN_IK narrow(rotated_model, rotated_lo, rotated_hi, rotated_tip_id, .03, 7e-7, mode);
      for (double target : {rotated_lo[0], rotated_hi[0]}) {
        angle[0] = target;
        pinocchio::forwardKinematics(rotated_model, rotated_data, angle);
        pinocchio::updateFramePlacements(rotated_model, rotated_data);
        const auto target_pose = rotated_data.oMf[rotated_tip_id];
        for (double initial : {target - 2*M_PI - .01, target + 2*M_PI + .01}) {
          Eigen::VectorXd initial_q(1), result;
          initial_q[0] = initial;
          if (narrow.CartToJnt(initial_q, target_pose, result) < 0) {
            std::cerr << "Narrow-limit mode=" << mode << " target=" << target << " seed=" << initial << '\n';
            throw std::runtime_error("Narrow-limit IK failed");
          }
          require(result[0] >= rotated_lo[0] && result[0] <= rotated_hi[0], "Narrow-limit wrapping failed");
          require(std::abs(result[0]-target) < 1e-6, "Narrow-limit solution changed pose");
        }
      }
    }
