...
def generate(gap=None, out=None, position_actuators=False):
    if gap is not None:
        WALLS["gap"] = float(gap)
    ...
    if position_actuators:
        # Optional 20k/500 servo experiment. Keep each control range inside
        # its joint limits; the steering-wheel joints remain passive.
        actuators = ET.SubElement(model, "actuator")
        for suffix in "LR":
            for axis in range(1, 8):
                joint_name = f"AR5-5_07{suffix}-W4C4A2_joint_{axis}"
                joint = world.find(f".//joint[@name='{joint_name}']")
                assert joint is not None and joint.get("range")
                ET.SubElement(actuators, "position", name=f"{joint_name}_position",
                              joint=joint_name, kp="20000", kv="500",
    ...
    # The wheel joints deliberately have neither actuators nor YAML drivers.
    ET.indent(model, space="  ")
    default_name = "aviator_position_20k500.xml" if position_actuators else "aviator.xml"
    out_path = Path(out) if out else PROJECT / "model" / default_name
...
if __name__ == "__main__":
    ...
    ap.add_argument("--position-actuators", action="store_true",
                    help="write separate 14-actuator kp=20000 kv=500 test model")
    args = ap.parse_args()
    generate(gap=args.gap, out=args.out, position_actuators=args.position_actuators)
