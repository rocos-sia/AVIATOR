...
namespace rocos {
    ...
    void Robot::on_fsm_reset() {
        ...
        if (!parseUrdf(urdf_file_path_, base_link_, tip_)) {
            log_ptr_->error("Robot initialization failed: could not parse URDF or set kinematic chain");
            impl_->process_event(EventErrorOccurred{});
            return;
        }
        ...
        if (!externally_scheduled_) startMotionThread();
    ...
    Robot::Robot(HardwareInterface *hw,
                 ...
                 const string &tip, bool externally_scheduled
    ) : externally_scheduled_(externally_scheduled), hw_interface_(hw), urdf_file_path_(urdf_file_path), base_link_(base_link), tip_(tip), pos_(MAX_JOINT_NUM),
    ...
    Robot::~Robot() {
        // 停止运动线程，避免 std::terminate
        is_running_motion_ = false;
    ...
    bool Robot::parseUrdf(const string &urdf_file_path,
        ...
        if (externally_scheduled_ || !loader.loadFromYAML("robotDH.yaml"))
        {
            if (!externally_scheduled_) log_ptr_->warn("No DH parameters; using URDF chain");
        ...
        // 动力学链为非关键路径，加载失败时仅警告，不影响机器人基本功能
        if (!dynamics_.setChain(tree, base_link, tip))
        {
            log_ptr_->warn("Could not set dynamics chain, continuing without dynamics.");
        }
        else
        {
    ...
    bool Robot::parseDriveParamsFromUrdf(const string &urdf_file_path) {
        ...
        // 遍历运动链的所有 segment；固定安装段不占关节，但不能截掉末端驱动。
        for (unsigned i = 0; i < kinematics_.getChain().getNrOfSegments(); ++i) {
            const auto& chain_joint = kinematics_.getChain().getSegment(i).getJoint();
            if (chain_joint.getType() == KDL::Joint::None) continue;
            for (auto element = robot->FirstChildElement("joint"); element;
                 element = element->NextSiblingElement("joint")) {
                if (element->Attribute("name") && element->Attribute("name") == chain_joint.getName()) {
                    const auto *joint_name = element->Attribute("name");

                    auto hw = element->FirstChildElement("hardware");
                    if (!hw || !hw->FirstChildElement("limit") || !hw->FirstChildElement("transform")) {
                        log_ptr_->error("Missing hardware parameters for {}", joint_name);
                        return false;
                    }

                    auto id = hw->IntAttribute("id", -1); // 对应的硬件ID，若没指定默认为-1
                    if (id < 0 || id >= hw_interface_->getSlaveNumber()) return false;
        ...
        return joints_.size() == static_cast<size_t>(jnt_num_);
    }
    ...
    void Robot::motionThreadHandler() {
        ...
        while (is_running_motion_) {  // while start
    ...
    int Robot::Dragging(DRAGGING_FLAG flag, DRAGGING_DIRRECTION dir, double max_speed, double max_acceleration) {
        ...
        if (index <= static_cast< int >( DRAGGING_FLAG::J7 ))  //当前命令类型为关节空间
        {
            ...
            //预防关节数量不足时，下发超出范围的关节控制命令
            if (index >= jnt_num_) {
                log_ptr_->error(" command flag index={} is not allowed because jnt_num_={}", index, jnt_num_);
                return -1;
            }
        ...
        // 拖拽完成时，归还机器人状态（RUNNING → STOPPED），防止下次拖拽被阻塞
        if (_dragging_finished_flag && isMotionRunning() && dir == DRAGGING_DIRRECTION::NONE) {
            enterStopped();
        }
